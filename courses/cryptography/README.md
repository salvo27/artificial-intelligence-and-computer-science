# Cryptography

| | |
|---|---|
| **Code** | 27006172 |
| **ECTS** | 6 |
| **Period** | Year 1, semester 1 |
| **Curriculum** | Computer Security |
| **Lecturer** | J. van Bon |
| **Exam format** | _TBD_ |
| **Official page** | _TBD_ |

## Contents

- [Summary](summary.md)
- [Lecture notes](notes/)
- [Exercises and past exams](exercises/): [algebra](exercises/algebra.md) (sections 1.1, 1.2 and 1.3)

## Syllabus

From the index of the lecturer's notes. Links point to the matching section of the [summary](summary.md).

**1. Algebra**

- [1.1 Basic notions](summary.md#11-basic-notions)
- [1.2 The ring Z/mZ](summary.md#12-the-ring-zmz)
- [1.3 Direct products and homomorphisms](summary.md#13-direct-products-and-homomorphisms)
- 1.4 Groups
- 1.5 Polynomial rings
- 1.6 Finite fields
- 1.7 Linear algebra revisited
- 1.8 Prime numbers

**2. Cryptography**

- 2.1 Basic notions
- 2.2 AES
- 2.3 RSA
- 2.4 ElGamal
- 2.5 Elliptic curves

## Resources

- **Lecture notes by J. van Bon**, handed out by the lecturer during the course. They are not included
  in this repository: ask the lecturer or check the course's official channels.

<!-- Textbooks, links to the lecturers' slides (do not upload the files), videos, useful repos. -->
